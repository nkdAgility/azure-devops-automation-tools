function Invoke-AutomationWorkspaceInit {
    [CmdletBinding()]
    param(
...
$materialiseFromGallery = { param($capability, $resolved, $modulePath)
    ...
    $existingManifest = Join-Path $modulePath "$module.psd1"
    $compatibleCopy = $module -ne 'NKDAgility.AzureDevOps.AutomationTools' -or
        ((Test-Path -LiteralPath $existingManifest) -and
         (Select-String -LiteralPath $existingManifest -SimpleMatch "'Invoke-AutomationWorkspaceInit'" -Quiet))
    if ($NoSync -and (Test-Path -LiteralPath $modulePath) -and $compatibleCopy) {
    ...
    $haveTag = if ($have) { $have.PrivateData.PSData.Prerelease }
    $haveVersion = if ($have) { "$($have.Version)" + $(if ($haveTag) { "-$($haveTag.TrimStart('-'))" } else { '' }) }
    if ($wanted -and (-not $have -or $haveVersion -ne "$($wanted.Version)")) {
...
if ($selfUpdated -and -not $env:AZDO_INIT_RELOADED) {
    ...
    try { . $InitScriptPath -NoSync:$NoSync }
...
$runtimeManifest = Join-Path $automation.ModulePath 'NKDAgility.AzureDevOps.AutomationTools.psd1'
$runtimeModule = Import-Module $runtimeManifest -Force -Global -PassThru
$runtimeWorkspace = & $runtimeModule.ExportedCommands['Initialize-AutomationWorkspace'] -Path $workspaceRoot
...
$secretsPath = $runtimeWorkspace.SecretsPath
$preserveCiCredentials = $env:CI -in @('true', '1')
& $runtimeModule.ExportedCommands['Set-AutomationSecrets'] -SecretsPath $secretsPath -NoClobber:$preserveCiCredentials -ClearMissing | Out-Null
