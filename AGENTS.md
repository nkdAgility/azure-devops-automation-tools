...
| `system/…/Templates/customer-repo/` | Customer workspace scaffold, laid out relative to the workspace root (`init.ps1`, `capabilities.json`, `workspace.json`, `gitignore.template` → `.gitignore`, customer `AGENTS.md`, `Codex.managed.md`, `.Codex/`, `secrets/secrets.example.json`). `.managed` lists which of them this engine owns and overwrites every session; everything else is a seed |
| `system/…/Templates/migrations/` | Per-type engagement templates: `data-import/` (Scratchbook + Cleanup runbooks), `migration-tools/` (Sync + Run-* binders + configs), `migration-platform/` (Sync + platform-config), `github-repos/` (inventory + Sync) |
...
| `system/…/Agents/CAPABILITY.md` | Guidance for agents *using* this capability in a workspace. Rendered into the workspace's `AGENTS.md`, `AGENTS.md` and `.github/copilot-instructions.md`. Contrast with this file, which is for agents *building* the toolkit |
...
`PreToolUse` hook in the workspace's `.Codex/settings.json` that refuses agent writes into
...
| ---- | --------- | -------- |
| **Seed** | copied once at scaffold time, then owned by the customer | engagement runbooks, `workspace.json`, `data/organisations.json`, the customer's own `AGENTS.md` prose |
| **Managed** | overwritten from the module on every client `init.ps1` | `init.ps1`, `secrets/secrets.example.json`, the `<!-- BEGIN managed: automation-tools -->` block in the customer `AGENTS.md` |
...
`AGENTS.md` in a client repo is **co-owned**: the customer's prose is a seed, and only the marked
...
| `Get-AzureDevOpsAccessToken` | Entra access token for a collection — usable anywhere a PAT is (REST Bearer, git http.extraheader). Cached, renewed near expiry. Entra-first callers try this and fall back to a PAT only when it throws |
...
| `Get-GitHubAccessToken` | Resolves a GitHub token: the signed-in gh CLI first, then `GITHUB_TOKEN` — the GitHub counterpart of the Entra default. Throws with guidance when neither yields one |
...
Auto memory is disabled for this project (`.Codex/settings.json`). Persistent memory lives in `.Codex/memory/` instead — it is gitignored because it may reference client engagements. At the start of a session, read `.Codex/memory/MEMORY.md` (if it exists) and follow its links for context; update those files the same way you would auto memory (one fact per file, index line in MEMORY.md).
