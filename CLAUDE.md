...
| `system/…/Engines/` | Standalone migration engines invoked by the `Run-Migrate-*.ps1` binders: `Migrate-Repos.ps1` (git repos incl. LFS/segmented pushes) and `Migrate-Artifacts.ps1` (artifact feeds/packages). They live in the module so they travel into `.system/` with it and stay locked to the module version that drives them; binders resolve them from `ModuleBase\Engines`, never by walking up |
